...
local function getTagFunc(tagstr)
	...
	if(not func) then
		local frmt, num = tagstr:gsub('%%', '%%%%'):gsub(_PATTERN, '%%s')
		local funcs = {}
		...
		for bracket in tagstr:gmatch(_PATTERN) do
				...
				if(tag) then
					if(prefixEnd ~= 1 or suffixStart - suffixEnd ~= 1) then
						...
						tagFunc = function(unit, realUnit)
							local str
							if(customArgs) then
								str = tag(unit, realUnit, split(',', customArgs))
							else
			...
			if(not tagFunc) then
				nierror(format('Attempted to use invalid tag %s.', bracket))
		...
		func = function(self)
			...
			self:SetFormattedText(frmt, unpack(buffer, 1, num))
		end
