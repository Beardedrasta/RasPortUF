...
local CastingBarFrameSetUnit = _G.CastingBarFrame_SetUnit
local CastingBarFrameOnLoad = _G.CastingBarFrame_OnLoad
local PetCastingBarFrameOnLoad = _G.PetCastingBarFrame_OnLoad
...
local function Enable(self, unit)
	...
	if(element and unit and not unit:match('%wtarget$')) then
		...
		if(self.unit == 'player' and not (self.hasChildren or self.isChild or self.isNamePlate)) then
			CastingBarFrameSetUnit(CastingBarFrame, nil)
			CastingBarFrameSetUnit(PetCastingBarFrame, nil)
		end
...
local function Disable(self)
	...
	if(element) then
		...
		if(self.unit == 'player' and not (self.hasChildren or self.isChild or self.isNamePlate)) then
			CastingBarFrameOnLoad(CastingBarFrame, 'player', true, false)
			PetCastingBarFrameOnLoad(PetCastingBarFrame)
		end
	end
end
