...
local SecureButtonGetUnit = _G.SecureButton_GetUnit
local SecureButtonGetModifiedUnit = _G.SecureButton_GetModifiedUnit
...
local oUF_Parent = CreateFrame('Frame', (global or parent) .. '_Parent', UIParent, 'SecureHandlerStateTemplate')
oUF_Parent:SetAllPoints()
oUF_Parent:SetFrameStrata('LOW')
RegisterStateDriver(oUF_Parent, 'visibility', '[petbattle] hide; show')

local function updateActiveUnit(self, event)
	-- Calculate units to work with
	local realUnit, modUnit = SecureButtonGetUnit(self), SecureButtonGetModifiedUnit(self)
...
do
	...
	* oUF-onlyProcessChildren   - can be used to force headers to only process children (boolean?)
	...
	function oUF:SpawnHeader(overrideName, template, visibility, ...)
		...
		local header = CreateFrame('Frame', name, oUF_Parent, template)
...
function oUF:Spawn(unit, overrideName)
	...
	local object = CreateFrame('Button', name, oUF_Parent, 'SecureUnitButtonTemplate')
