...
function AceTab:RegisterTabCompletion(descriptor, prematches, wordlist, usagefunc, listenframes, postfunc, pmoverwrite)
	...
	elseif type(listenframes) ~= 'table' or type(listenframes[1]) == 'userdata' and type(listenframes.IsObjectType) == 'function' then  -- single frame or framename
	...
	for _, f in pairs(listenframes) do
		...
		if type(f) ~= 'table' or type(f[1]) ~= 'userdata' or type(f.IsObjectType) ~= 'function' then
