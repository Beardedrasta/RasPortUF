local MINOR = 8
local lib, minor = LibStub('LibDropDown-ls')
if minor > MINOR then
	return
end
...
function buttonStretchMixin:OnMouseDown()
	if self:IsEnabled() then
		...
		self.Arrow:AdjustPointsOffset(1, -1)
		PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON)
	end
end
...
function buttonStretchMixin:SetArrowPosition(side, dir, offset)
	...
	if side == 'left' then
		self.Arrow:SetPoint('LEFT', 5, 0)
	else
		self.Arrow:SetPoint('RIGHT', -5, 0)
	end

	if dir == 'top' then
		self.Arrow:SetTexCoord(1, 0, 0, 0, 1, 1, 0, 1)
		self:SetAnchor('BOTTOMLEFT', self, 'TOPLEFT', offset or 10, 20)
	elseif dir == 'bottom' then
		self.Arrow:SetTexCoord(0, 1, 1, 1, 0, 0, 1, 0)
		self:SetAnchor('TOPLEFT', self, 'BOTTOMLEFT', offset or 10, -20)
	elseif dir == 'left' then
		self.Arrow:SetTexCoord(1, 0, 0, 1)
		self:SetAnchor('TOPRIGHT', self, 'TOPLEFT', -20, -(offset or 10))
	else
		self.Arrow:SetTexCoord(0, 1, 0, 1)
		self:SetAnchor('TOPLEFT', self, 'TOPRIGHT', 20, -(offset or 10))
	end
end
